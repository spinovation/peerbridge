...
export default function TaxModule({ state }) {
  ...
  const taxDocuments = documentation.filter(doc => doc.doc_type === 'tax_document');
  ...
  const ordinaryDividends = taxDocuments.reduce((acc, curr) => acc + (curr.dividends || 0), 0);
  const qualifiedDividends = ordinaryDividends * 0.8; // 80% qualified
  ...
  return (
    <div style={styles.container} className="animate-fade-in-up">
      <div style={styles.introHeader}>
        <h2 style={styles.title}>Tax Reporting & Compliance Center</h2>
        <p style={styles.sub}>Access annual 1099 forms, dividend ledgers, and satisfy IRS Reg D reporting criteria.</p>
      </div>
      ...
      <div style={styles.grid}>
        ...
        <div style={styles.leftCol}>
          ...
          <div style={styles.statsRow}>
            <div className="glass-panel" style={styles.statCard}>
              <span style={styles.statLabel}>Fiscal Year Capital Placed</span>
              <span style={styles.statVal}>${totalPlacedCapital.toLocaleString()}</span>
            </div>
            <div className="glass-panel" style={styles.statCard}>
              <span style={styles.statLabel}>Ordinary Dividends (Est)</span>
              <span style={styles.statVal}>${ordinaryDividends.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
          </div>

          {/* Tax Documents from DB Table #7 */}
          <div className="glass-panel" style={styles.card}>
            <h3 style={styles.cardTitle}>📄 Compiled IRS Tax Documents (Table #7)</h3>
            <p style={styles.cardDesc}>Download verified tax forms compiled from your Peer Bridge P2P transaction history.</p>

            <div style={styles.docList}>
              {taxDocuments.length === 0 ? (
                <div style={styles.emptyBoxText}>
                  <p>No tax documents compiled yet. Invest in crowdfunding campaigns to automatically generate IRS Form 1099s.</p>
                </div>
              ) : (
                    ...
                    <div style={styles.docMeta}>
                      ...
                      <div>
                        <h4 style={styles.docName}>
                          Form 1099-DIV – {doc.companyName || 'Dividend Distribution'}
                        </h4>
                        <span style={styles.docDate}>
                          Compiled for {taxYear} Tax Season • {doc.uploaded_at ? new Date(doc.uploaded_at).toLocaleDateString() : 'Active'}
                        </span>
                      </div>
                    </div>
                    <button
        ...
        <div className="glass-panel" style={styles.rightColPanel}>
          ...
          <p style={styles.rightText}>
            Under IRS regulations, dividends from crowdfunding SPVs are reported using Form 1099-DIV, while direct peer-to-peer interest distributions are tracked via 1099-INT.
          </p>
          <p style={styles.rightText}>
          ...
          <div style={styles.complianceBadge}>
            🛡 IRS Reg D Compliant Node
          </div>
        </div>
      </div>

      {/* High-Fidelity IRS Form 1099-DIV Interactive Preview */}
      {show1099Preview && selectedTaxDoc && (
        <div style={styles.backdrop}>
          <div className="glass-panel glow-accent-border" style={styles.form1099Card}>
            <div style={styles.formHeader}>
              <div style={styles.formHeaderLeft}>
                <h3 style={styles.formTitle}>Form 1099-DIV</h3>
                <span style={styles.formSub}>OMB No. 1545-0110</span>
              </div>
            ...
            <h4 style={styles.formSectionTitle}>Dividends and Distributions</h4>
            ...
            <div style={styles.irsGrid}>
              ...
              <div style={styles.irsBox}>
                ...
                <strong style={styles.boxVal}>${(selectedTaxDoc.dividends || 0).toFixed(2)}</strong>
              </div>
              ...
              <div style={styles.irsBox}>
                <span style={styles.boxLabel}>5 Investment expenses ($)</span>
                <strong style={styles.boxVal}>$0.00</strong>
              </div>
            </div>

            <div style={styles.disclaimer}>
              ⚠ This represents a verified compliance simulation card. Real IRS Form filings must be processed by certified tax affiliates.
            </div>
          </div>
        </div>
      )}
